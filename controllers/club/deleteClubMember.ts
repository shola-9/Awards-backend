...
export const deleteClubMember: express.RequestHandler = async (
  req: Request,
  res: Response
): Promise<void> => {
    ...
    connection.query(
      ...
      (err, result) => {
        if (err) {
          ...
          res.status(404).json({ message: "Club members not found" });
        } else {
          res
            .status(200)
            .json({ message: "User deleted from club", clubPosts: result });
        }
      }
    );
  } catch (error) {
